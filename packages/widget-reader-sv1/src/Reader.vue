      ...
      <skeleton>
        ...
        <template slot="body">
          <div class="passage-heading">
            <h1>
              <template v-for="(breadcrumb, idx) in text.metadata.ancestors">
                <a :key="breadcrumb.urn" :href="breadcrumb.url">{{
                  breadcrumb.label
                }}</a
                ><template v-if="idx != text.metadata.ancestors.length - 1"
                  >,
                </template>
              </template>
            </h1>
            <template v-if="!rightPassage">
              ...
              <h3>
                <passage-human-reference :metadata="leftPassage.metadata" />
              </h3>
            </template>
          </div>
          <version-selector
            v-if="canSelectVersions"
            :versions="versions"
            :to="toRightPassage"
          >
          ...
          <div
            id="overall"
            class="overall"
            :dir="text.metadata.rtl ? 'rtl' : 'ltr'"
          >
            <div :class="lowerPassageText ? 'upper' : 'upper-lower'">
              <div class="pg-left">
                <router-link
                  v-if="passage.metadata.prev"
                  :to="toRef(passage.metadata.prev.ref)"
                >
                  <span>
              ...
              <template v-if="rightUrn">
                <div class="left">
                  ...
                  >
                    ...
                    <div class="metadata">
                      {{ leftText.metadata.human_lang }}
                      {{ leftText.metadata.kind }}
                    </div>
                  </version-selector>
                  <div
                    v-if="leftPassage.error"
                    class="alert text-danger"
                    role="alert"
                  >
                    Failed to load <b>{{ leftPassage.urn.toString() }}</b
                    >: {{ leftPassage.error }}
                  </div>
                  <template v-else>
                    <passage-redirect-notice
                      v-if="leftPassage.redirected"
                      :passage="leftPassage"
                    />
                    <passage-render-text
                ...
                <div class="right">
                  <template v-if="rightText && rightText.metadata">
                    <version-selector
                      :versions="versions"
                      :to="toRightPassage"
                      :remove="toRemoveRight"
                    >
                      {{ rightText.metadata.label }}
                      <div class="metadata">
                        {{ rightText.metadata.human_lang }}
                        {{ rightText.metadata.kind }}
                      </div>
                    </version-selector>
                    <div
                      v-if="rightPassage.error"
                      class="alert text-danger"
                      role="alert"
                    >
                      Failed to load <b>{{ rightPassage.urn.toString() }}</b
                      >: {{ rightPassage.error }}
                    </div>
                    <template v-else>
                      <passage-redirect-notice
                        v-if="rightPassage.redirected"
                        :passage="rightPassage"
                      />
                      <passage-render-text
                        :text="rightPassageText"
                        :highlighting="false"
                      />
                    </template>
                  </template>
              ...
              <div class="pg-right">
                <router-link
                  v-if="passage.metadata.next"
                  :to="toRef(passage.metadata.next.ref)"
                >
                  <span>
  ...
  export default {
    ...
    computed: {
      ...
      versions() {
        ...
        return (
          versions &&
          Object.keys(versions.collections).map(
            key => versions.collections[key],
          )
        );
      },
      canSelectVersions() {
